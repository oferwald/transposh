...
PHPFILES=`find . -maxdepth 4 -iname '*.php' -not -path "./build/*" -not -path "./scripts/*" -not -path "./resources/*" -not -path "./test/*"`
...
if [ $MINIFY == true ]; then
  ...
  for file in `find . -maxdepth 2 -iname '*.css' -not -name 'circle-flags.css' -not -name 'flags.css' -not -name 'square-flags.css'`; do
    echo "minifying $file"
